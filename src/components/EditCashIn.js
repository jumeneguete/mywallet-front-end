...
export default function EditCashIn() {
  ...
  useEffect(() => {
    ...
    const result = axios.get(
      `${process.env.REACT_APP_HOST}/register/${id}`,
      config
    );
  ...
  function addRegister(e) {
    ...
    if (amount <= 0)
      return alert("Você deve adicionar entradas com valores positivos!");

    const value = Number(amount).toFixed(2) * 100;
    const body = { value, description };

    const request = axios.post(
      `${process.env.REACT_APP_HOST}/updatecashin/${id}`,
      body,
      config
    );
    setLoading(true);
  ...
  function deleteRegister() {
    const result = axios.delete(
      `${process.env.REACT_APP_HOST}/delete/${id}`,
      config
    );
    ...
    <>
      <Container>
        ...
        <form onSubmit={addRegister}>
          ...
          />
          <Button type="submit">
            {loading ? <ThreeDots /> : "Salvar entrada"}
          </Button>
          <DeleteButton onClick={deleteRegister}>
            {loadingDelete ? <ThreeDots /> : "Excluir entrada"}
          </DeleteButton>
        </form>
      </Container>
      <Link to="/home">
